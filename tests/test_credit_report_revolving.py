...
def test_recover_revolving_multiline_with_split_institution() -> None:
    raw_text = """
信贷记录明细
未结清信贷
循环透支 共 1 笔
B10411000H
0001310470715627312662
中国建设银行 股份有限公司 上海五角场支 行流动资金贷款 2024-02-22 2025-08-21 人民币元 460 新增
公共记录明细
抵押 454.68 正常 0 0 0 2025-03-06
78.06 正常还款 5 --B10411000H0001310470715627312662 见附件 2025-03-31
"""
    normalized = final_normalize_credit_result(
        {
            "credit_summary": {
        ...
        raw_text=raw_text,
        parser_path="pytest_split_revolving",
    )
    ...
    assert record["end_date"] == "2025-08-21"
    assert record["five_category"] == "正常"
    assert str(record["overdue_months"]) == "0"
