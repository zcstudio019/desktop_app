...
def test_first_rows_prefer_table_cells_when_page_text_omits_comparison_column() -> None:
    pages = [
        {
            "page": 1,
            "text": """资产负债表
...
""",
            "table_rows": [
                ["销售产成品、商品、", "提供劳务收到的现金", "1", "81,530,980.95", "14,260,100.00"],
            ],
        },
    ]
    result = run_financial_report_agent(
        raw_text="\n".join(page["text"] for page in pages),
        filename="小企业首行-cells回填.pdf",
        metadata={"raw_pages": pages},
    )
    ...
    assert balance["normalized_value"] == 119011.57
    assert balance["previous_normalized_value"] == 4803622.66
    assert balance["source_text"] == "货币资金 1 119,011.57 4,803,622.66"
    assert cashflow["normalized_value"] == 81530980.95
...
def test_small_business_balance_sheet_summary_shows_non_zero_rows_and_original_opening_amounts() -> None:
    pages = [{
        "page": 1,
        "text": """小企业会计准则 资产负债表
资产 行次 期末金额 年初余额
...
周转材料 13 0.00 0.00
流动资产合计 16 45,484,417.03 18,003,977.90
长期股权投资 19 600,000.00 600,000.00
固定资产原价 20 43,975,106.41 43,975,106.41
累计折旧 21 21,748,765.79 19,318,200.04
固定资产账面价值 22 22,226,340.62 24,656,906.37
在建工程 23 16,434,684.01 16,434,684.01
工程物资 23 0.00 0.00
固定资产清理 24 0.00 0.00
无形资产 26 27,287,834.45 29,996,163.45
开发支出 27 0.00 0.00
长期待摊费用 28 16,852,968.79 8,668,858.35
非流动资产合计 30 83,401,827.87 80,356,612.18
资产总计 31 231,536,626.49 195,677,269.12
短期借款 32 2,000,000.00 0.00
应付账款 34 20,413,003.45 15,793,885.12
应付职工薪酬 35 498,547.00 459,352.00
应交税费 36 420,494.59 342,975.26
流动负债合计 44 42,046,587.04 17,143,638.50
负债合计 52 47,017,886.39 17,965,478.50
实收资本 53 101,800,000.00 101,800,000.00
资本公积 54 3,044,947.91 3,044,947.91
盈余公积 55 2,957,779.78 2,957,779.78
未分配利润 56 46,021,386.55 38,963,174.52
所有者权益合计 59 153,824,114.24 146,765,902.21
负债和所有者权益总计 60 231,536,626.49 195,677,269.12
""",
    }]
    result = run_financial_report_agent(
        raw_text=pages[0]["text"],
        filename="小企业资产负债表.pdf",
        metadata={
            "raw_pages": pages,
            "historical_financial_reports": [{
                "company_info": {"report_period_end": "2025-12-31"},
                "balance_sheet": {"cash_and_equivalents": {"normalized_value": 44541.34}},
            }],
        },
    )
    balance = result["structured_json"]["balance_sheet"]
    ...
    assert balance["cash_and_equivalents"]["normalized_value"] == 119011.57
    assert balance["cash_and_equivalents"]["previous_normalized_value"] == 4803622.66
    assert balance["short_term_investments"]["normalized_value"] == 0.00
    assert balance["short_term_investments"]["previous_normalized_value"] == 0.00
    assert balance["notes_receivable"]["normalized_value"] == 0.00
    assert balance["notes_receivable"]["previous_normalized_value"] == 0.00
    ...
    assert "| 长期股权投资 | 600,000.00 | 600,000.00 | 1 | 0.96 |" in markdown
    assert "| 固定资产原价 | 43,975,106.41 | 43,975,106.41 | 1 | 0.96 |" in markdown
    assert "| 累计折旧 | 21,748,765.79 | 19,318,200.04 | 1 | 0.96 |" in markdown
    assert "| 固定资产账面价值 | 22,226,340.62 | 24,656,906.37 | 1 | 0.96 |" in markdown
    assert "| 在建工程 | 16,434,684.01 | 16,434,684.01 | 1 | 0.96 |" in markdown
    assert "| 无形资产 | 27,287,834.45 | 29,996,163.45 | 1 | 0.96 |" in markdown
    assert "| 长期待摊费用 | 16,852,968.79 | 8,668,858.35 | 1 | 0.96 |" in markdown
    assert "| 非流动资产合计 | 83,401,827.87 | 80,356,612.18 | 1 | 0.96 |" in markdown
    assert "| 短期借款 | 2,000,000.00 | 0.00 | 1 | 0.96 |" in markdown
    assert "| 应付账款 | 20,413,003.45 | 15,793,885.12 | 1 | 0.96 |" in markdown
    assert "| 应付职工薪酬 | 498,547.00 | 459,352.00 | 1 | 0.96 |" in markdown
    assert "| 应交税费 | 420,494.59 | 342,975.26 | 1 | 0.96 |" in markdown
    assert "| 负债合计 | 47,017,886.39 | 17,965,478.50 | 1 | 0.96 |" in markdown
    assert "| 实收资本 | 101,800,000.00 | 101,800,000.00 | 1 | 0.96 |" in markdown
    assert "| 资本公积 | 3,044,947.91 | 3,044,947.91 | 1 | 0.96 |" in markdown
    assert "| 盈余公积 | 2,957,779.78 | 2,957,779.78 | 1 | 0.96 |" in markdown
    assert "| 未分配利润 | 46,021,386.55 | 38,963,174.52 | 1 | 0.96 |" in markdown
    assert "| 负债和所有者权益总计 | 231,536,626.49 | 195,677,269.12 | 1 | 0.96 |" in markdown
    for hidden_label in (
        "短期投资", "应收票据", "应收股利", "应收利息", "在产品", "库存商品",
        "周转材料", "工程物资", "固定资产清理", "开发支出",
    ):
        assert f"| {hidden_label} |" not in markdown
    assert "| 货币资金 | 119,011.57 | 44,541.34 |" not in markdown


def test_single_report_detail_does_not_fill_missing_comparison_from_previous_report() -> None:
    pages = [{"page": 1, "text": "资产负债表\n货币资金 1 119,011.57"}]
    result = run_financial_report_agent(
        raw_text=pages[0]["text"],
        filename="当前单份报表.pdf",
        metadata={
