...
async def _load_comprehensive_export_snapshot(customer_id: str, report_id: str, current_user: dict):
    customer = await storage_service.get_customer(customer_id)
    if not customer:
        raise HTTPException(status_code=404, detail="未找到该客户")
    await _ensure_local_customer_access(customer, current_user)
    getter = getattr(storage_service, "get_comprehensive_financing_report_snapshot", None)
    snapshot = await getter(customer_id, report_id) if callable(getter) else None
    if not snapshot:
        raise HTTPException(status_code=404, detail="未找到该综合融资报告版本，请先在 AI 助手生成报告")
    return snapshot


@router.get("/{customer_id}/comprehensive-financing-report/snapshots/{report_id}/preview")
async def preview_comprehensive_financing_report(customer_id: str, report_id: str, current_user: dict = Depends(get_current_user)):
    from backend.services.comprehensive_financing_report_export_service import snapshot_html
    snapshot = await _load_comprehensive_export_snapshot(customer_id, report_id, current_user)
...
@router.get("/{customer_id}/comprehensive-financing-report/snapshots/{report_id}/export/pdf")
async def export_comprehensive_financing_report_pdf(customer_id: str, report_id: str, current_user: dict = Depends(get_current_user)):
    from backend.services.comprehensive_financing_report_export_service import (
        snapshot_html, pdf_filename, render_html_pdf, CreditReportExportUnavailable,
    )
    snapshot = await _load_comprehensive_export_snapshot(customer_id, report_id, current_user)
    try:
