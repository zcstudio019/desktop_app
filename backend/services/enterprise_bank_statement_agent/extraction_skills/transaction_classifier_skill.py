...
def _known_accounts(metadata: dict[str, Any], transactions: list[dict[str, Any]]) -> set[str]:
    ...
    for key in ("known_account_numbers", "knownAccounts", "account_numbers", "accountNumbers", "self_account_numbers"):
        value = metadata.get(key)
        if isinstance(value, str):
...
def _name_in_list(name: str, values: set[str]) -> bool:
    if not name:
        return False
    normalized = normalize_company_name(name)
    return normalized in values or any(normalized and (normalized in item or item in normalized) for item in values)
...
def classify_transaction_nature(
    ...
    *,
    ...
    rules = rules or {}
    manual_overrides = rules.get("manual_overrides") if isinstance(rules.get("manual_overrides"), dict) else {}
    tx_id = str(tx.get("transaction_id") or "")
    override = manual_overrides.get(tx_id)
    if isinstance(override, dict):
        return {
            "nature": override.get("nature") or "unknown",
            "exclude_from_operating": bool(override.get("exclude_from_operating")),
            "reason": override.get("manual_reason") or "人工复核覆盖",
            "confidence": 1.0,
    ...
    whitelist = {normalize_company_name(item) for item in rules.get("operating_counterparty_whitelist") or [] if item}
    blacklist = {normalize_company_name(item) for item in rules.get("internal_counterparty_blacklist") or [] if item}
    personal_names = {normalize_company_name(item) for item in rules.get("personal_counterparty_names") or [] if item}
    configured_keywords = tuple(rules.get("internal_transfer_keywords") or TRANSFER_WORDS)

    if any(_name_in_list(name, whitelist) for name in names):
        return {
            "nature": "operating",
            "exclude_from_operating": False,
            "reason": "命中经营性客户白名单，按真实经营流水计入",
            "confidence": 1.0,
            "manual_reviewed": False,
        }
    if any(_name_in_list(name, blacklist) for name in names):
        return {
            "nature": "internal_transfer",
            "exclude_from_operating": True,
            "reason": "命中内部往来黑名单，强制剔除",
            "confidence": 1.0,
            "manual_reviewed": False,
        }
    if any(_name_in_list(name, personal_names) for name in names):
        return {
            "nature": "personal_transfer",
            "exclude_from_operating": False,
            "reason": "命中个人往来名单，列示为可疑个人往来",
            "confidence": 0.95,
            "manual_reviewed": False,
        }
    ...
    transfer_hint = any(word in text for word in configured_keywords)
...
def classify_transactions(transactions: list[dict[str, Any]], company_name: str | None = None, metadata: dict[str, Any] | None = None) -> list[dict[str, Any]]:
    ...
    rules = metadata.get("enterprise_flow_rules") if isinstance(metadata.get("enterprise_flow_rules"), dict) else metadata
    for item in rules.get("related_company_names") or []:
        if item:
            aliases.add(normalize_company_name(item))
    known_accounts = _known_accounts(metadata, transactions)
    for item in rules.get("self_account_numbers") or []:
        normalized = normalize_account_number(item)
        if normalized:
            known_accounts.add(normalized)
    ...
    for tx in transactions:
        ...
        nature = classify_transaction_nature(tx, aliases=aliases, known_accounts=known_accounts, rules=rules)
        tx["nature"] = nature["nature"]
        tx["exclude_from_operating"] = bool(nature["exclude_from_operating"])
        tx["nature_reason"] = nature["reason"]
        tx["classification_reason"] = nature["reason"]
        tx["nature_confidence"] = nature["confidence"]
        tx["classification_confidence"] = nature["confidence"]
        tx["manual_reviewed"] = bool(nature.get("manual_reviewed"))
        tx["review_status"] = "reviewed" if tx["manual_reviewed"] else ("pending" if tx["exclude_from_operating"] or tx["nature"] in {"related_party", "personal_transfer"} else "auto")
