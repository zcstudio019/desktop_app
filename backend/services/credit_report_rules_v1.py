...
def evaluate_credit_report_rules(report_model: dict[str, Any]) -> list[dict[str, str]]:
    ...
    if any(value > 0 for value in overdue_values):
    ...
    if enterprise_guarantee is not None and enterprise_guarantee > 0:
        ...
        guarantee_item = _item("企业对外担保", "达标", "余额：0", "企业征信明确记录为0", "持续关注新增对外担保")
    else:
    ...
    return [overdue, overdue_90_item, query_item, card_item, dti_item, online_item, concentration_item, guarantee_item, card_count_item]
