...
def _legacy_from_agent_revolving(item: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "last_repayment_amount": _fmt_amount(item.get("last_repayment_amount")) if item.get("last_repayment_amount") is not None else "",
        "last_repayment_type": item.get("last_repayment_type"),
        "remaining_repayment_months": item.get("remaining_repayment_months"),
        "credit_agreement_no": item.get("credit_agreement_no"),
        "history_performance": item.get("history_performance"),
