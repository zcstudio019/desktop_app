...
def _extract_credit_card_balance(block: str) -> str:
    source = _normalize_block(block)
...
def _merge_recovered_card(existing: dict[str, Any], recovered: dict[str, Any]) -> None:
    for key in ("credit_limit", "balance", "used_limit", "used_amount", "special_installment_balance", "account_status", "report_cutoff", "card_type", "currency", "open_date"):
...
def parse_credit_card_account_block(block: str) -> dict[str, Any]:
    ...
    balance = ""
    if not used_limit:
        balance = _extract_credit_card_balance(source)
        if balance:
            card_type = "准贷记卡"
            logger.info(
                "[PersonalCredit][CreditCard][BALANCE_RECORD_AS_QUASI_CARD] issuer=%s balance=%s card_type=%s",
                institution,
                balance,
                card_type,
            )
...
def extract_credit_card_accounts(sections: dict[str, Any]) -> list[dict[str, Any]]:
    try:
        ...
        for index, block in enumerate(_candidate_blocks(text), start=1):
            ...
            if record.get("balance") and "余额" in str(record.get("evidence_text") or "") and "已使用额度" not in str(record.get("evidence_text") or ""):
                logger.info(
                    "[PersonalCredit][CreditCard][BALANCE_RECORD_AS_QUASI_CARD] index=%s issuer=%s balance=%s card_type=%s",
                    index,
                    record.get("issuer") or record.get("institution"),
                    record.get("balance"),
        ...
        if not records and _expected_active_credit_card_count(source_text) > 0:
            ...
            for index, block in enumerate(_active_card_sentence_blocks(fallback_window), start=1):
                ...
                if record.get("balance") and "余额" in str(record.get("evidence_text") or "") and "已使用额度" not in str(record.get("evidence_text") or ""):
                    logger.info(
                        "[PersonalCredit][CreditCard][BALANCE_RECORD_AS_QUASI_CARD] index=fallback-%s issuer=%s balance=%s card_type=%s",
                        index,
                        record.get("issuer") or record.get("institution"),
                        record.get("balance"),
                        record.get("card_type"),
