...
def render_personal_bank_statement_markdown(data: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- 可采信收入：{_money(income.get('verified_income') or income.get('stable_income'))}",
        f"- 月均可采信收入：{_money(income.get('avg_monthly_verified_income'))}",
        "",
        "## 工资收入识别",
        f"- 明确工资收入：{_money(income.get('confirmed_salary_income'))}",
        ...
        f"- 月均工资：{_money(income.get('salary_avg_monthly_amount'))}",
        f"- 月均可采信工资：{_money(income.get('avg_monthly_verified_income'))}",
