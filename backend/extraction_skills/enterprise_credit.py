    ...
    def extract(self, input_data: ExtractionInput) -> ExtractionResult:
        ...
        try:
            ...
            report_date = _extract_by_patterns(
                raw_text,
                (
                    r"(?:报告时间|报告日期|查询日期)[：:\s]*(" + DATE_PATTERN.pattern + r")",
                ),
            )
            ...
            established_date = _extract_by_patterns(
                raw_text,
                (
                    r"(?:成立日期|成立时间|注册日期)[：:\s]*(" + DATE_PATTERN.pattern + r")",
                ),
            )
            ...
            logger.exception("enterprise_credit_skill_extract_failed file=%s error=%s", input_data.file_name, e)
            _safe_print("[EnterpriseCreditSkill]", "提取失败", str(e))
            return ExtractionResult(
                document_type="enterprise_credit",
                schema_version="enterprise_credit.v1",
                extracted_json={},
